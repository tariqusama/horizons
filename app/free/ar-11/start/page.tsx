...
export default function AR11Page() {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Alien's Change of Address Card", pw / 2, y + 5, { align: "center" });
    
    doc.setFontSize(10);
    doc.text("Department of Homeland Security", pw / 2, y + 10, { align: "center" });
    doc.text("U.S. Citizenship and Immigration Services", pw / 2, y + 14, { align: "center" });
    
    doc.setFontSize(11);
    doc.text("USCIS", pw - m, y + 5, { align: "right" });
    doc.text("Form AR-11", pw - m, y + 10, { align: "right" });
    ...
    const drawSection = (title: string, currentY: number) => {
      ...
      doc.rect(m, currentY, pw - 2 * m, 6, 'DF');
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.text(title, m + 2, currentY + 4.5);
      return currentY + 10;
    };
    
    const drawField = (label: string, value: string, x: number, currentY: number, width: number, height: number = 6) => {
      ...
      doc.rect(x, currentY + 1.5, width, height, 'S');
      
      if (value) {
        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);
        const splitValue = doc.splitTextToSize(value, width - 2);
        doc.text(splitValue, x + 2, currentY + 5.5);
      }
      return currentY + height + 6;
    };

    const drawCheckboxGroup = (label: string, options: string[], selectedValue: string, x: number, currentY: number) => {
      doc.setFont("times", "normal");
      doc.setFontSize(8);
      doc.text(label, x, currentY);
      let currX = x;
      options.forEach(opt => {
        doc.setDrawColor(0);
        doc.setLineWidth(0.3);
        doc.rect(currX, currentY + 1.5, 4, 4, 'S');
        if (selectedValue === opt) {
          doc.setFont("helvetica", "bold");
          doc.setFontSize(10);
          doc.text("X", currX + 1, currentY + 4.5);
        }
        doc.setFont("times", "normal");
        doc.setFontSize(8);
        doc.text(opt, currX + 5, currentY + 4.5);
        currX += 13;
      });
    };
    ...
    drawField("*Family Name (Last Name)", formData.familyName || "", m, y, 60);
    drawField("*Given Name (First Name)", formData.givenName || "", m + 63, y, 60);
    drawField("Middle Name (if applicable)", formData.middleName || "", m + 126, y, 60);
    
    y += 12;
    
    drawField("*Date of Birth (mm/dd/yyyy)", formData.dob || "", m, y, 40);
    
    doc.setFont("times", "normal");
    doc.setFontSize(8);
    doc.text("Alien Registration Number (A-Number) (if any)", m + 45, y);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.text("► A-", m + 45, y + 5.5);
    drawField("", formData.aNumber || "", m + 53, y, 45); // Draw A-Number box without label overlapping
    
    y += 12;
    
    // --- Information About Your Address ---
    y = drawSection("Information About Your Address", y + 2);
    ...
    drawField("*Street Number and Name", formData.pres_street || "", m, y, 98);
    drawCheckboxGroup("Apt. Ste. Flr.", ["Apt.", "Ste.", "Flr."], formData.pres_unit_type || "", m + 102, y);
    drawField("Number", formData.pres_unit_number || "", m + 148, y, 38);
    
    y += 12;
    
    drawField("*City or Town", formData.pres_city || "", m, y, 98);
    drawField("*State", formData.pres_state || "", m + 102, y, 20);
    drawField("*ZIP Code", formData.pres_zip || "", m + 126, y, 60);
    ...
    drawField("Street Number and Name", formData.prev_street || "", m, y, 98);
    drawCheckboxGroup("Apt. Ste. Flr.", ["Apt.", "Ste.", "Flr."], formData.prev_unit_type || "", m + 102, y);
    drawField("Number", formData.prev_unit_number || "", m + 148, y, 38);
    
    y += 12;
    
    drawField("City or Town", formData.prev_city || "", m, y, 98);
    drawField("State", formData.prev_state || "", m + 102, y, 20);
    drawField("ZIP Code", formData.prev_zip || "", m + 126, y, 60);
    ...
    drawField("Street Number and Name", formData.mail_street || "", m, y, 98);
    drawCheckboxGroup("Apt. Ste. Flr.", ["Apt.", "Ste.", "Flr."], formData.mail_unit_type || "", m + 102, y);
    drawField("Number", formData.mail_unit_number || "", m + 148, y, 38);
    
    y += 12;
    
    drawField("City or Town", formData.mail_city || "", m, y, 98);
    drawField("State", formData.mail_state || "", m + 102, y, 20);
    drawField("ZIP Code", formData.mail_zip || "", m + 126, y, 60);
    ...
    drawField("*Your Signature", agreed ? `${fullName} (Digitally Signed)` : "", m, y, 120);
    const today = new Date().toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' });
    drawField("Date of Signature (mm/dd/yyyy)", today, m + 125, y, 61);
    ...
    doc.text("Page 1 of 1", pw - m, ph - m, { align: "right" });
