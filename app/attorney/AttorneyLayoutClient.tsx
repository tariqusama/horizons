...
const navItems = [
    { label: 'Dashboard', href: '/attorney', bg: '#FDECE2', fg: '#F97316' },
    { label: 'Assigned Cases', href: '/attorney/assigned-cases', bg: '#ECFDF5', fg: '#16A34A' },
    { label: 'Pending Reviews', href: '/attorney/pending-reviews', bg: '#EFF6FF', fg: '#2563EB' },
    { label: 'Messages', href: '/attorney/messages', bg: '#FEF3C7', fg: '#A16207' },
    { label: 'Client Files', href: '/attorney/client-files', bg: '#F8FAFC', fg: '#0F766E' },
];
    ...
    return (
        <>
            ...
            <div className="px-6 pb-3">
                <span className="text-xs font-semibold text-slate-500 tracking-widest">ATTORNEY PORTAL</span>
            </div>

            <div className="flex flex-col flex-1 min-h-0 px-3">
                <nav className="space-y-1">
                    {navItems.map((item) => {
                        const isActive = item.href === '/attorney' ? pathname === '/attorney' : pathname?.startsWith(item.href);
                        return (
                            <Link
                                key={item.label}
                                href={item.href}
...
export default function AttorneyLayoutClient({ children }: { children: React.ReactNode }) {
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50">
                <div className="text-center text-slate-600">Loading attorney portal…</div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
                <div className="max-w-md w-full rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
                    ...
                    <p className="text-sm text-slate-600 mb-6">You do not have access to the attorney portal.</p>
                    <button
            ...
            <div className="flex-1 min-w-0 flex flex-col lg:ml-[19rem]">
                <header className="flex items-center gap-2 sm:gap-4 px-3 sm:px-4 lg:px-6 py-3 sm:py-4 mb-4 bg-white rounded-3xl shadow-sm">
                    ...
                    <div className="flex-1 relative min-w-0 hidden sm:block">
                        ...
                        <input
                            type="text"
                            placeholder="Search cases, clients, messages..."
                    ...
                    <Link href="/dashboard/profile" className="hidden sm:flex items-center gap-3 px-4 py-2 rounded-full bg-white border border-slate-200 shadow-sm hover:border-slate-300 transition">
                        <div className="h-10 w-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 text-white flex items-center justify-center font-bold">{user?.name?.charAt(0) ?? 'A'}</div>
                        <div className="min-w-0">
                            <p className="text-sm font-semibold text-slate-900 truncate">{user?.name || 'Attorney'}</p>
                            <p className="text-[11px] text-slate-500">Attorney portal</p>
                        </div>
                    </Link>
                </header>
